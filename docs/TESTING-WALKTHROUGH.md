...
►► Chapter 3 UAT: Complete Persistence Scenarios
  ℹ️  Test File: tests/ch3_uat.nim
  ...
  ⚡ Command: nim c -r tests/ch3_uat.nim
...
nim c --compileOnly tests/api_tests.nim

# Build with optimizations
nim c -d:release -r tests/ch3_uat.nim

# Run with verbose output (if test supports it)
nim c -r tests/integration_test.nim -v
```
...
nim c -r tests/api_tests.nim && `
nim c -r tests/config_test.nim
...
2. Run with fresh compile (don't use cached objects):
   ```powershell
   nim c --forceBuild -r tests/validation_test.nim
   ```
...
2. Try: `nim c -d:release -r tests/validation_firewall_perf_test.nim`
...
- **Test examples:** Examples in `tests/example_test.nim`
