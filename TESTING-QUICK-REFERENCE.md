...
```powershell
# Run one test
nim c -r tests/ch3_uat.nim

# Just compile (check for errors)
nim c --compileOnly tests/validation_test.nim
...
3. **Why?** → Read the test code (tests/*.nim)
...
- **Full Guide:** `docs/TESTING-WALKTHROUGH.md`
- **Architecture:** `docs/PLAN.md`
- **Test Code:** `tests/ch*_uat.nim` (start with these)
- **Examples:** `tests/example_test.nim`
