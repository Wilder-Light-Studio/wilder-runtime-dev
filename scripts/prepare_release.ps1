...
if (-not (Test-Path -LiteralPath $binaryPath)) {
  & (Join-Path $resolvedRoot "scripts/build_binary.ps1") -RootPath $resolvedRoot -OutputPath "bin/cosmos.exe" -ReleaseTag $tag
}
...
& (Join-Path $resolvedRoot "scripts/generate_artifact_checksums.ps1") -Files $artifacts
...
foreach ($artifactPath in $artifacts) {
  ...
  & (Join-Path $resolvedRoot "scripts/generate_release_manifest.ps1") `
